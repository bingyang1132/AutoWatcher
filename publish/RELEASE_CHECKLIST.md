...
- [x] 上传创意工坊 `3797303841`（2026-09-14）。中英两份标题描述都传了，三个依赖没动。
      更新记录已上（09-14 18:15）。
      **踩了一个小坑：`changeNote` 写成了 markdown 的 `**粗体**`，Steam 不渲染，原样显示星号。**
...
- [x] tag `v1.0.6` 已推；GitHub release
      [`v1.0.6`](https://github.com/bingyang1132/AutoWatcher/releases/tag/v1.0.6) 已建，
      附 `AutoWatcher-1.0.6.zip`（DLL + 清单 + 第三方声明）。这次 `gh release create` 没被拦。
- [ ] **全量 34 条没有重跑。** 这一版只动一个新补丁点和预见的公共前几步，其余夹具不沾边；
      要保险可以补跑一轮。

## 1.0.7（2026-09-17）

只改一样东西：清单里的 `author` 从 `bingyang1132` 换成 Steam 昵称 **机动战士高俅**，和工坊条目的
作者名一致。没有任何功能改动。

之所以为此单开一版而不是原地重传 1.0.6：同一个版本号对应同一份字节，1.0.6 的 GitHub release
附件已经发出去了，再悄悄换掉工坊那份内容会让两边同号不同物。

- [x] `AutoWatcher.json` / `AutoWatcher.csproj` 版本 `1.0.7`，`author` 换名。
- [x] 上传创意工坊 `3797303841`（2026-09-17）。`changeNote` 写明只改作者名、不用重新验证。
- [x] tag `v1.0.7` 与 GitHub release。
- 正文没有加 ★ 更新公告：作者名不是玩家能感觉到的改动，公告位留给真正的修复。
