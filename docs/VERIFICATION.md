...
**9 个遗物**分三类，只有一类需要注册镜像：阳、美琅脂、斗篷扣走求解器会分发的钩子，已注册；
清水和泪滴挂坠在战斗开始前触发，求解器的根快照取在战斗开始之后，已经反映了结果，**不该**
补镜像否则会重复；黄金眼和紫色莲花自己什么都不重写，是按遗物 ID 轮询的，已分别在预见张数和
...
那几张（张数已过黄金眼和守视），下界 0 上界全选，于是"丢掉"和"留着"都是正常的世界线分支，
...
登记的是**相对通用估值的偏置**，两张都取 `-10`。

**负值是必须的，不是调参。** `ChoicePriority` 对消耗返回 `-Σ RemovalPriority` 并按降序取分支，
估值全为正时「一张都不选」（0）恒排第一——消耗在选择排序里只有「少亏一点」、没有正收益。这就是
第一版（只把打击从 6.0 压到 4.0）实机毫无变化的原因：净化和一堆打击一起在手上，它就是不烧。
`-10` 把打击压到 `-4`、防御压到 `-6`，两张都低于零，「烧它」这条分支才排得到「不烧」前面。

**两张同一个偏置就够。** 通用估值把格挡打了八折，所以防御（`4.0 - 10`）自然排在打击
（`6.0 - 10`）前面被烧。观者靠姿态、心灵堡垒和警惕起甲，一张普通防御确实比一张打击更该烧——
和原版五个角色相反，这也是不走「起手打击/防御」类别抽象的原因：那个抽象会把原版的假设强加过来。
升级梯度也保住了，打击+ 9 伤害是 `-1`，仍然比未升级的那张更靠后烧。

入口可选：绑不上就跳过，行为和登记之前一样，加载日志里写明。

**这只解决「别烧错、该烧的要烧」。** 求解器仍然没有「压牌库」这个正收益项（`liveDeckClutter`
只数状态牌和诅咒），所以它不会为了凑无限而主动多烧；南瓜、钢铁闪光这类同样是死牌的也仍然不会被
优先烧。那一层是求解器的估值主干，已另行提 issue 讨论。
...
- **登记入口是可选的，但「可选」不等于「绑不上也没关系」。** 求解器没有 `GrowthSourceMirrors`
  时两个登记都返回空，收益照旧只走长期资源刻度，加载日志里写明「求解器没有这个入口」。

### 这个探测断过一次，而且是静默断的（2026-09-15）

`BindGrowthSourceRegister` 原来要求 `Register` **正好 4 个参数**。求解器 `0.38.3`
（*feat: stop search at proven growth targets*）在末尾加了第 5 个**可选**参数
`opportunityTarget`，于是这个探测从那一版起一直返回 `null`——两张牌的成长额度悄悄没了两天。

失败的形状值得记下来：

- **没有任何报错。** 不抛异常，路线上不打红字，矩阵也不挂（见下）。
- **日志那句话把人带偏了。** 旧版探测只有两态，绑不上一律写成「求解器没有这个入口」，
  于是「上游还没做」和「上游改了签名、我们过期了」长得一模一样。查这个问题时，
  这句话先骗了一轮。
- **矩阵抓不到。** `WATCHER-LESSON-LEARNED-FATAL-KILL` 断言的是
  `-ExpectedInitialLongTermResourceValueAtLeast 30`，而长期资源刻度**正是入口绑不上时的回退
  路径**。夹具锁的是回退值，不是成长额度，所以入口断了它照样通过。

1.0.5 的改法：只核对前 4 个参数的类型，后面多出来的**可选**参数一律容忍（上游在末尾加可选参数
是正常的加功能方式），非可选的多余参数才算真对不上；调用侧按实际参数个数建实参数组。
同时把探测结果从两态改成四态——已绑定 / 上游没有 / **签名对不上** / 探测出错——后两种额外单独
告警一次。

**仍然没有夹具。** 需要一条能断言「成长额度被兑现」的判据，而不是断言回退刻度。
矩阵里也还没有「给某个来源设一份额度再看路线变不变」的参数。现有证据只有编译期加签名核对两级，
实机行为靠加载日志那一行确认。
...
| `WATCHER-SCRY-SHUFFLES-EMPTY-DRAW` | 抽牌堆空、弃牌堆四张时，天眼的预见仍然开出选牌分支（要先洗牌才看得到牌） | 通过 |
| `WATCHER-FASTING-ENERGY-DOWN` | 挂着斋戒的能量减益时，36 血敌人要打到第三回合才死（不扣那 1 费的话第二回合就该斩杀） | 通过 |
...
### 矩阵 30/30（2026-09-09，求解器 0.34.7）

第一次跑全量，也是求解器从 0.29 升到 0.34.7 之后第一次跑。30 条里 29 条直接通过，
`WATCHER-MIRACLE-ENERGY` 挂了一次，查下来是**判据过期，不是镜像坏了**。

原判据是「首轮至少 3 个可执行动作」：敌人 80 血、手里奇迹加两张爆发，赌求解器会打奇迹补能量
凑出第二张爆发。实测首轮只有 **1** 个动作——它打一张爆发就结束回合，把奇迹留在手里，主动卖
4 血分三回合打完，整场 8 血。

镜像是好的（`StarterDeckMirrors.Miracle` 照常 `GainEnergy(owner, Energy.BaseValue)`）。变的是
求解器的取舍：80 血那版本来就杀不掉，留着一张 0 费保留牌比换这一回合的节奏更值。这类判据吃
路线偏好，而路线偏好在 0.29 到 0.34.7 之间改过好几轮。

改成和兄弟夹具一样的「正好斩杀」：敌人 27 血，爆发 9 点后进愤怒，奇迹补 1 点能量凑出第二张，
第二张吃翻倍打 18 点，9 + 18 = 27。奇迹的能量没建模就只有 9 点。

- 正向：通过。
- 反向对照：把 `Miracle` 里的 `GainEnergy` 注掉重新构建，挂在「首轮预计战斗结束回合为 3，预期为 1」；
  恢复后重新通过。

**教训：判「动作数」「最高可起防」这类量的时候要想清楚它吃不吃路线偏好。**「正好斩杀」和
「某个数值的算术下限」不吃，「求解器会不会选择做某件事」吃。
...
**二、预见回收迂回。** 判据从 `Id.Entry == "WEAVE"` 改成 `card is WatcherWeave`。迂回的真实
id 是 `WATCHER_WEAVE`，所以 0.9.27 那个字符串**从来没匹配上**，实机压根没把迂回收回来；
...
### 矩阵 32/32（2026-09-10，求解器 0.34.8 + 观者 0.9.28）

一批跑完，**0 失败、0 未运行**，21 分钟，单条约 39 秒（0.34.7 那会儿是 ~52 秒，开机变快了）。

意义在于这是求解器和观者**同时换版之后**的第一次全量：0.34.7 → 0.34.8 动了临时力量／敏捷／
集中的施加顺序、卡牌克隆、若干 Power 顺序；观者 0.9.27 → 0.9.28 动了通晓万物和预见回收迂回。
30 条老夹具一条没挂，说明这两批改动都没有波及已经核过的部分。

新增的两条通晓万物夹具见上一节，反向对照也在那里。
