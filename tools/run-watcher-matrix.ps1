...
param(
    ...
    [string]$Tag = "",
    [string]$ProgressPath = "",
    # 整个矩阵的硬时限。harness 卡住时会一直耗到自己的 150 秒上限，25 条全卡就是一小时。
    # 到点就停下并把没跑的列成"未运行"，不要让它无声地拖下去。
    #
    # 单条用例的耗时基本全是开机：求解器 0.31.1 那会儿一条 37 秒，0.31.3 变成 67 秒，
    # 而且是每一条都均匀变慢、跟夹具内容无关。25 条按 67 秒算是 28 分钟，所以默认给 40 分钟。
    # 25 分钟那次跑完 21 条就到点了（21 条全过，剩 4 条未运行）——别把"未运行"当成通过。
    [int]$MaxTotalMinutes = 40,
    [switch]$NoRestart
)
